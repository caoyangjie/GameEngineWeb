<template>
	<div class="unifi-lock-history-container">
	  <!-- 背景层 -->
	  ...
	  <!-- 主要内容区域 -->
	  <main class="unifi-lock-history-main-content">
		<!-- 标题 -->
		<div class="title-section">
		  <div class="title-banner">
			<span class="title-icon">💰</span>
			<span class="title-text">{{ t('unifiLockHistory.title') }}</span>
		  </div>
		</div>
		...
		<div class="filter-section">
		  <div class="filter-row">
			<div class="filter-field">
			  <label class="filter-label">{{ t('unifiLockHistory.startDate') }}</label>
			...
			<div class="filter-field">
			  <label class="filter-label">{{ t('unifiLockHistory.endDate') }}</label>
			  <div class="filter-input-wrapper">
				<input 
				  type="date" 
		  ...
		  <button class="filter-button" @click="applyFilter">
			{{ t('unifiLockHistory.applyFilter') }}
		  </button>
		</div>
		...
		<div class="history-table-section">
		  ...
		  <div class="table-header">
			<div class="header-cell">{{ t('unifiLockHistory.date') }}</div>
			<div class="header-cell">{{ t('unifiLockHistory.amount') }}</div>
			<div class="header-cell">{{ t('unifiLockHistory.unifiPrice') }}</div>
			<div class="header-cell">{{ t('unifiLockHistory.percentage') }}</div>
			<div class="header-cell">{{ t('unifiLockHistory.purchaseValue') }}</div>
		  </div>
		  ...
		  <div class="table-content">
			...
			<div v-else class="table-rows">
			  ...
			  >
				...
				<div class="table-cell">{{ formatUnifiPrice(bounty.unifiPrice) }}</div>
				<div class="table-cell">{{ formatPercentage(bounty.percentage) }}</div>
				<div class="table-cell">{{ formatPurchaseValue(bounty.purchaseValue) }}</div>
			  </div>
			</div>
		  </div>
		</div>
	  ...
	  <Sidebar 
		:is-open="sidebarOpen" 
		active-route="unifi-lock-history"
  ...
  // UNIFI锁仓数据（示例数据，实际应该从API获取）
  const bounties = ref([
	{
	  id: 1,
	  date: '2025-12-11 00:00:16',
	  amount: '12.62981050',
	  unifiPrice: '0.30000000',
	  percentage: '0.050',
	  purchaseValue: '7577.88600000'
	},
	{
	  id: 2,
	  date: '2025-11-11 00:00:16',
	  amount: '12.64878366',
	  unifiPrice: '0.30000000',
	  percentage: '0.050',
	  purchaseValue: '7589.27000000'
	},
	{
	  id: 3,
	  date: '2025-10-11 00:00:16',
	  amount: '12.66778533',
	  unifiPrice: '0.30000000',
	  percentage: '0.050',
	  purchaseValue: '7600.67100000'
	},
	{
	  id: 4,
	  date: '2025-09-11 00:00:15',
	  amount: '12.68681550',
	  unifiPrice: '0.30000000',
	  percentage: '0.050',
	  purchaseValue: '7612.08900000'
	},
	{
	  id: 5,
	  date: '2025-08-11 00:00:16',
	  amount: '12.70587416',
	  unifiPrice: '0.30000000',
	  percentage: '0.050',
	  purchaseValue: '7623.52500000'
	},
	{
	  id: 6,
	  date: '2025-07-11 00:00:16',
	  amount: '12.72496150',
	  unifiPrice: '0.30000000',
	  percentage: '0.050',
	  purchaseValue: '7634.97700000'
	},
	{
	  id: 7,
	  date: '2025-06-11 00:00:16',
	  amount: '12.74407750',
	  unifiPrice: '0.30000000',
	  percentage: '0.050',
	  purchaseValue: '7646.44700000'
	},
	{
	  id: 8,
	  date: '2025-05-11 00:00:16',
	  amount: '12.76322216',
	  unifiPrice: '0.30000000',
	  percentage: '0.050',
	  purchaseValue: '7657.93300000'
	},
	{
	  id: 9,
	  date: '2025-04-11 00:00:16',
	  amount: '12.78239550',
	  unifiPrice: '0.30000000',
	  percentage: '0.050',
	  purchaseValue: '7669.43700000'
	},
	{
	  id: 10,
	  ...
	  unifiPrice: '0.30000000',
	  percentage: '0.050',
	  purchaseValue: '7680.95800000'
	},
	{
	  id: 11,
	  date: '2025-02-11 00:00:16',
	  amount: '12.82082816',
	  unifiPrice: '0.30000000',
	  percentage: '0.050',
	  purchaseValue: '7692.49700000'
	}
  ])
  ...
  const formatAmount = (amount) => {
	if (!amount) return ''
	// 格式化金额，保持8位小数精度，无千位分隔符
	const num = parseFloat(amount)
	if (isNaN(num)) return amount
	// 使用 toFixed(8) 确保8位小数，不添加千位分隔符
	return num.toFixed(8)
  }
  
  const formatUnifiPrice = (price) => {
	if (!price) return ''
	// 格式化UNIFI价格，保持8位小数精度，无千位分隔符
  ...
  /* 主要内容区域 */
  .unifi-lock-history-main-content {
	position: relative;
	z-index: 5;
	padding: 40px;
	max-width: 1400px;
  ...
  .table-header {
	display: grid;
	grid-template-columns: 1.5fr 1.2fr 1.2fr 0.9fr 1.5fr;
	gap: 10px;
	padding: 20px;
