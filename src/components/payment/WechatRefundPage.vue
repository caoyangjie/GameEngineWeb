  ...
  <div class="payment-container">
    ...
    <main class="payment-main-content">
      <div class="payment-panel">
        <div class="payment-title">{{ t('payment.wechat.refund.title') }}</div>
        ...
        <form @submit.prevent="handleSubmit" class="payment-form">
          ...
          <div class="form-row">
            <div class="form-group">
              <label class="form-label">{{ t('payment.wechat.refund.outRefundNo') }} <span class="required">*</span></label>
              <CustomInput
                v-model="formData.outRefundNo"
                :placeholder="t('payment.wechat.refund.outRefundNoPlaceholder')"
                required
              />
            </div>
            <div class="form-group">
              <label class="form-label">{{ t('payment.wechat.refund.totalFee') }} <span class="required">*</span></label>
              <CustomNumberInput
          ...
          <div class="form-group full-width">
            <label class="form-label">{{ t('payment.wechat.refund.refundFee') }} <span class="required">*</span></label>
            <CustomNumberInput
              v-model="formData.refundFee"
              :min="0.01"
              :placeholder="t('payment.wechat.refund.refundFeePlaceholder')"
              required
            />
          </div>
          
          <button type="submit" class="submit-button refund">
            <span>{{ t('payment.common.applyRefund') }}</span>
          </button>
...
import { getPaymentConfigList, wechatRefund } from '../../api/payment.js'
...
const generateRefundNo = () => {
  return 'REFUND' + new Date().getTime() + Math.floor(Math.random() * 1000)
}

onMounted(() => {
  formData.outRefundNo = generateRefundNo()
  loadConfigs()
})
...
const handleSubmit = async () => {
  try {
    const response = await wechatRefund({
      configId: formData.configId,
      outTradeNo: formData.outTradeNo,
      outRefundNo: formData.outRefundNo,
    ...
    if (response.code === 200 && response.data) {
      result.value = {
        ...
        message: response.data.message || (response.data.success ? t('payment.common.refundSuccess') : t('payment.common.refundFailed')),
        data: response.data
      }
    } else {
      result.value = {
        success: false,
        message: response.msg || t('payment.common.refundFailed')
      }
    }
  } catch (error) {
    result.value = {
