  ...
  <div class="payment-container">
    ...
    <main class="payment-main-content">
      <div class="payment-panel">
        <div class="payment-title">{{ t('payment.wechat.unifiedOrder.title') }}</div>
        ...
        <form @submit.prevent="handleSubmit" class="payment-form">
          ...
          <div class="form-row">
            <div class="form-group">
              <label class="form-label">{{ t('payment.common.subject') }} <span class="required">*</span></label>
              <CustomInput
            ...
            <div class="form-group">
              <label class="form-label">{{ t('payment.common.totalAmount') }} <span class="required">*</span></label>
              <CustomNumberInput
          ...
          <div class="form-row">
            <div class="form-group">
              <label class="form-label">{{ t('payment.wechat.unifiedOrder.tradeType') }} <span class="required">*</span></label>
              <CustomSelect
            ...
            <div class="form-group">
              <label class="form-label">{{ t('payment.wechat.unifiedOrder.openid') }}</label>
              <CustomInput
          ...
          <div class="form-group full-width">
            <label class="form-label">{{ t('payment.common.body') }}</label>
            <CustomTextarea
          ...
          <button type="submit" class="submit-button">
            <span>{{ t('payment.wechat.unifiedOrder.submit') }}</span>
          </button>
          ...
          <div v-if="result" :class="['result-container', result.success ? 'result-success' : 'result-error']">
            ...
            <div v-if="result.codeUrl" class="qrcode-container">
...
import { getPaymentConfigList, wechatUnifiedOrder } from '../../api/payment.js'
...
onMounted(() => {
  formData.outTradeNo = generateOutTradeNo()
  formData.subject = t('payment.common.subjectPlaceholder')
  formData.body = t('payment.common.bodyPlaceholder')
  loadConfigs()
})

const loadConfigs = async () => {
  try {
    const response = await getPaymentConfigList('WECHAT')
...
const handleSubmit = async () => {
  try {
    const response = await wechatUnifiedOrder({
      configId: formData.configId,
      outTradeNo: formData.outTradeNo,
      subject: formData.subject,
      body: formData.body || formData.subject,
    ...
    if (response.code === 200 && response.data) {
      result.value = {
        ...
        message: response.data.message || (response.data.success ? t('payment.common.operationSuccess') : t('payment.common.operationFailed')),
        codeUrl: response.data.codeUrl,
        data: response.data
      }
      
      if (response.data.codeUrl && qrcodeCanvas.value) {
        nextTick(() => {
...
.payment-panel {
  ...
  box-shadow: 
    0 0 30px rgba(7, 193, 96, 0.3),
    inset 0 0 30px rgba(7, 193, 96, 0.05);
  max-width: 840px;
  margin: 0 auto;
}
...
.submit-button {
  background: linear-gradient(135deg, rgba(7, 193, 96, 0.9) 0%, rgba(6, 173, 86, 0.9) 100%);
  color: white;
  border: none;
  border-radius: 8px;
  padding: 12px 40px;
