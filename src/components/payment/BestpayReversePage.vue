  ...
  <div class="payment-container">
    ...
    <main class="payment-main-content">
      <div class="payment-panel">
        <div class="payment-title">{{ t('payment.bestpay.reverse.title') }}</div>
        ...
        <form @submit.prevent="handleSubmit" class="payment-form">
          ...
          <button type="submit" class="submit-button cancel">
            <span>{{ t('payment.common.reverseOrder') }}</span>
          </button>
...
import { getPaymentConfigList, bestpayReverse } from '../../api/payment.js'
...
const handleSubmit = async () => {
  try {
    const response = await bestpayReverse({
    ...
    if (response.code === 200 && response.data) {
      result.value = {
        ...
        message: response.data.message || (response.data.success ? t('payment.common.reverseSuccess') : t('payment.common.reverseFailed')),
        data: response.data
      }
    } else {
      result.value = {
        success: false,
        message: response.msg || t('payment.common.reverseFailed')
      }
    }
  } catch (error) {
    result.value = {
