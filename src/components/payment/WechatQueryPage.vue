  ...
  <div class="payment-container">
    ...
    <main class="payment-main-content">
      <div class="payment-panel">
        <div class="payment-title">{{ t('payment.wechat.query.title') }}</div>
        ...
        <form @submit.prevent="handleSubmit" class="payment-form">
          ...
          <button type="submit" class="submit-button">
            <span>{{ t('payment.common.queryOrder') }}</span>
          </button>
          ...
          <div v-if="result" :class="['result-container', result.success ? 'result-success' : 'result-error']">
            ...
            <div v-if="result.tradeState" class="result-info">
              <div class="info-item">
                <span class="info-label">{{ t('payment.common.tradeState') }}</span>
                <span class="info-value">{{ result.tradeState }}</span>
              </div>
            </div>
...
import { getPaymentConfigList, wechatQuery } from '../../api/payment.js'
...
const handleSubmit = async () => {
  try {
    const response = await wechatQuery({
    ...
    if (response.code === 200 && response.data) {
      result.value = {
        ...
        message: response.data.message || (response.data.success ? t('payment.common.querySuccess') : t('payment.common.queryFailed')),
      ...
      result.value = {
        success: false,
        message: response.msg || t('payment.common.queryFailed')
      }
    }
  } catch (error) {
    result.value = {
