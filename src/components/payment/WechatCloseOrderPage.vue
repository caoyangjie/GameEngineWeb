  ...
  <div class="payment-container">
    ...
    <main class="payment-main-content">
      <div class="payment-panel">
        <div class="payment-title">{{ t('payment.wechat.closeOrder.title') }}</div>
        ...
        <form @submit.prevent="handleSubmit" class="payment-form">
          ...
          <button type="submit" class="submit-button cancel">
            <span>{{ t('payment.common.closeOrder') }}</span>
          </button>
...
import { getPaymentConfigList, wechatCloseOrder } from '../../api/payment.js'
...
const handleSubmit = async () => {
  try {
    const response = await wechatCloseOrder({
    ...
    if (response.code === 200 && response.data) {
      result.value = {
        ...
        message: response.data.message || (response.data.success ? t('payment.common.closeSuccess') : t('payment.common.closeFailed')),
        data: response.data
      }
    } else {
      result.value = {
        success: false,
        message: response.msg || t('payment.common.closeFailed')
      }
    }
  } catch (error) {
    result.value = {
