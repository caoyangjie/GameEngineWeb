  ...
  <div class="payment-container">
    ...
    <main class="payment-main-content">
      <div class="payment-panel">
        <div class="payment-title">{{ t('payment.alipay.preCreate.title') }}</div>
        ...
        <form @submit.prevent="handleSubmit" class="payment-form">
          ...
          <div class="form-row">
            <div class="form-group">
              <label class="form-label">{{ t('payment.common.storeId') }}</label>
              <CustomInput
            ...
            <div class="form-group">
              <label class="form-label">{{ t('payment.common.operatorId') }}</label>
              <CustomInput
          ...
          <button type="submit" class="submit-button">
            <span>{{ t('payment.common.generateQrcode') }}</span>
          </button>
          ...
          <div v-if="result" :class="['result-container', result.success ? 'result-success' : 'result-error']">
            ...
            <div v-if="result.qrcode" class="qrcode-container">
              <div class="qrcode-title">{{ t('payment.alipay.preCreate.qrcodeTitle') }}</div>
...
import { getPaymentConfigList, alipayPreCreate } from '../../api/payment.js'
...
const handleSubmit = async () => {
  try {
    const response = await alipayPreCreate({
    ...
    if (response.code === 200 && response.data) {
      ...
      if (response.data.qrcode && qrcodeCanvas.value) {
        nextTick(() => {
          QRCode.toCanvas(qrcodeCanvas.value, response.data.qrcode, {
            width: 300,
            margin: 2
          }, (error) => {
            if (error) {
