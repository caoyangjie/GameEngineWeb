<template>
	<div class="pass-card-management-container">
	  <!-- 背景层 -->
	  ...
	  <!-- 主要内容区域 -->
	  <main class="pass-card-management-main-content">
		<!-- 标题 -->
		  ...
		  <div class="table-content">
			...
			<div v-else class="table-rows">
			  <div 
				...
				class="table-row"
			  >
				<div class="table-cell">{{ coupon.type }}</div>
				<div class="table-cell">{{ coupon.code }}</div>
				<div class="table-cell">{{ coupon.effect }}</div>
				<div class="table-cell">{{ coupon.status }}</div>
				<div class="table-cell">{{ coupon.user }}</div>
				<div class="table-cell">{{ formatDateTime(coupon.usageDate) }}</div>
			  </div>
			</div>
		  </div>
		</div>
	  ...
	  <Sidebar 
		:is-open="sidebarOpen" 
		:active-route="ROUTES.PASS_CARD_MANAGEMENT"
  ...
  const coupons = ref([
	// 示例数据（当有数据时使用）
	{
	  ...
	  status: '已使用',
	  user: 'User001',
	  usageDate: '2024-12-11 10:30:00'
	}
  ])
  
  // 分页计算
  const totalItems = computed(() => coupons.value.length)
  ...
  <style scoped>
  .pass-card-management-container {
	position: relative;
	width: 100%;
	height: 100vh;
	overflow-y: auto;
  ...
  /* 主要内容区域 */
  .pass-card-management-main-content {
	position: relative;
	z-index: 5;
	padding: 40px;
	max-width: 1400px;
  ...
  .table-header {
	display: grid;
	grid-template-columns: 1fr 1.5fr 1.2fr 1fr 1fr 1.5fr;
	gap: 10px;
	padding: 20px;
  ...
  @media (max-width: 1200px) {
	...
	.table-header,
	.table-row {
	  grid-template-columns: 1fr 1.2fr 1fr 0.9fr 0.9fr 1.2fr;
	  font-size: 12px;
	}
