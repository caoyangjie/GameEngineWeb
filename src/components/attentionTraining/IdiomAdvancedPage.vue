<template>
  <div class="idiom-advanced-container">
    ...
    <main class="main-content">
      <header class="page-header">
        <div>
          <div class="badge">成语进阶 · 记忆力训练</div>
          <h1 class="page-title">成语快速记忆与语义回放</h1>
          <p class="subtitle">
            通过 DeepSeek 智能生成词单，结合背景故事与多维度记忆桩，完成“看卡记忆 → 语义回放”训练。
          </p>
        </div>
          ...
          <section class="controls">
            <div class="control-item wide">
              ...
              <textarea
                ...
                v-model="promptInput"
                rows="2"
                placeholder="例如：坚韧逆袭 / 处世之道 / 职场沟通 / 家风家训"
                :disabled="isLoadingIdioms || isRunning"
              ></textarea>
            </div>
            ...
            <div class="control-item">
              <label for="count">成语数量</label>
              <select id="count" v-model="idiomCount" :disabled="isLoadingIdioms || isRunning">
                <option value="4">4 条 · 速记</option>
                <option value="6">6 条 · 标准</option>
                <option value="8">8 条 · 进阶</option>
                <option value="12">12 条 · 挑战</option>
              </select>
            </div>
            <div class="control-item">
              <label for="difficulty">难度</label>
              <select id="difficulty" v-model="difficulty" :disabled="isLoadingIdioms || isRunning">
            ...
            <div class="control-actions">
              <button class="ghost" @click="loadLatestIdioms" :disabled="isLoadingIdioms || isRunning">
                用最近词单
              </button>
              <button class="primary" @click="generateAndStart" :disabled="isLoadingIdioms || isRunning">
                {{ isLoadingIdioms ? '生成中...' : '用 DeepSeek 生成词单' }}
              </button>
              <button class="btn-secondary" @click="openLexiconModal" :disabled="isLoadingIdioms">
                成语单词卡
              </button>
            </div>
          </section>

          <section class="game-card" v-if="idioms.length">
            <div class="game-content">
              <div class="exit-button-container">
                <button class="btn-exit" @click="resetGame" :disabled="isLoadingIdioms">重置</button>
              </div>

              <div v-if="gameStatus === 'memorizing'" class="memorizing-mode">
                <div class="group-header">
                  <div class="group-number">词单：{{ idioms.length }} 条</div>
                  <div class="timer" v-if="showTimer">{{ formatTime(memorizeRemaining) }}</div>
                </div>
                <div class="card-grid-display">
                  <div v-for="idiom in idioms" :key="idiomKey(idiom)" class="idiom-preview">
                    <div class="idiom-title">
                      <span class="main">{{ idiom.idiom }}</span>
                      <span class="pinyin">{{ idiom.pinyin }}</span>
                    </div>
                    <div class="idiom-meta">
                      <span>{{ idiom.originDynasty || '朝代未详' }}</span>
                      <span v-if="idiom.originSource">· {{ idiom.originSource }}</span>
                    </div>
                    <p class="idiom-text">{{ idiom.meaning }}</p>
                    <p class="idiom-text light">{{ idiom.moral }}</p>
                    <p class="idiom-text tiny">记忆线索：{{ idiom.memoryCues || '善用谐音 / 场景联想' }}</p>
                  </div>
                </div>
                <div class="hint-text">
                  先按故事/寓意给成语贴上场景，再在心里回放一遍。记好了可提前进入语义回放。
                </div>
              ...
              <div v-else-if="gameStatus === 'recalling'" class="recall-mode">
                ...
                <div v-if="currentQuestion" class="question-card">
                  ...
                  <div class="question-detail">
                    <span v-if="currentQuestion.idiom.originDynasty">{{ currentQuestion.idiom.originDynasty }}</span>
                    <span v-if="currentQuestion.idiom.protagonist"> · {{ currentQuestion.idiom.protagonist }}</span>
                  </div>
              ...
              <div v-else-if="gameStatus === 'completed'" class="result-card inline">
                ...
                <div class="result-stats">
                  <div class="stat-item">
                    ...
                    <div class="stat-value">{{ resultStats.total }} 条</div>
                  </div>
                  <div class="stat-item">
                    <div class="stat-label">答对</div>
                    <div class="stat-value success">{{ resultStats.correct }} 条</div>
                  </div>
                  <div class="stat-item">
    ...
    <Sidebar
      :is-open="sidebarOpen"
      :active-route="ROUTES.ATTENTION_IDIOM_ADVANCED"
        ...
        <div class="lexicon-body">
          <aside class="tag-panel">
            <div class="search-section">
              <div class="tag-title">成语检索</div>
              <div class="search-input-group">
                <input
                  v-model="searchInput"
                  type="text"
                  placeholder="输入成语名称进行检索"
                  class="search-input"
          ...
          <section class="lexicon-content">
            <div class="lexicon-list">
              ...
              <div class="idiom-list">
                <div
                  v-for="item in tagIdioms"
                  :key="idiomKey(item)"
                  class="idiom-row"
                  :class="{ 
                    active: selectedLexiconIdiom?.idiom === item.idiom,
                    'mode-focus': selectedLexiconIdiom?.idiom === item.idiom && lexiconMode === 'focus',
                    'mode-video': selectedLexiconIdiom?.idiom === item.idiom && lexiconMode === 'video'
                  }"
                >
                  <div class="idiom-row-content" @click="enterFocusMode(item)">
                    <div class="idiom-row-main">
                      <span class="lexicon-idiom-name">{{ item.idiom }}</span>
                      <span class="lexicon-idiom-pinyin">{{ item.pinyin }}</span>
                    </div>
                    <div class="idiom-row-meta">
                      {{ item.originDynasty || '朝代未详' }} · {{ item.moral || item.meaning || '待补充' }}
                    </div>
                  </div>
                  <button
                    v-if="item.videoUrl"
                    class="idiom-video-btn"
                    :class="{ active: selectedLexiconIdiom?.idiom === item.idiom && lexiconMode === 'video' }"
                ...
                <div v-if="!lexiconLoading && !tagIdioms.length" class="tag-empty">该标签暂无成语</div>
            ...
            <div class="lexicon-detail-wrapper">
              <!-- 视频记忆模式 -->
              <div class="video-mode" v-if="selectedLexiconIdiom && lexiconMode === 'video' && selectedLexiconIdiom.videoUrl">
                <div class="video-mode-header">
                  <div>
                    <div class="lexicon-idiom-name">{{ selectedLexiconIdiom.idiom }}</div>
                    <div class="lexicon-idiom-pinyin">{{ selectedLexiconIdiom.pinyin }}</div>
                  </div>
                  <button class="mode-switch-btn" @click="enterFocusMode(selectedLexiconIdiom)">
                    切换到专注记忆模式
                  </button>
                </div>
                <div class="video-container-full">
                  <iframe
                    v-if="isVideoUrl(getEmbedVideoUrl(selectedLexiconIdiom.videoUrl))"
                    :src="getEmbedVideoUrl(selectedLexiconIdiom.videoUrl)"
                    :title="`${selectedLexiconIdiom.idiom} 介绍视频`"
                  ...
                  <a
                    v-else
                    :href="selectedLexiconIdiom.videoUrl"
                    target="_blank"
                    rel="noopener noreferrer"
                    class="video-link-full"
                  >
                    点击观看视频：{{ selectedLexiconIdiom.videoUrl }}
                  </a>
                </div>
              </div>

              <!-- 专注记忆模式 -->
              <div class="lexicon-detail" v-else-if="selectedLexiconIdiom && lexiconMode === 'focus'">
                <div class="detail-header">
                  ...
                  <button 
                    v-if="selectedLexiconIdiom.videoUrl"
                    class="mode-switch-btn" 
                    @click="enterVideoMode(selectedLexiconIdiom)"
                  >
                    切换到视频记忆模式
                  </button>
                </div>
                <div class="detail-grid">
                  <div class="detail-block">
                    ...
                    <div class="detail-text">{{ selectedLexiconIdiom.meaning || '暂无' }}</div>
                  </div>
                  <div class="detail-block">
                    <div class="detail-label">寓意 / 启示</div>
                    <div class="detail-text">{{ selectedLexiconIdiom.moral || '暂无' }}</div>
                  </div>
                  <div class="detail-block">
                    <div class="detail-label">故事速记</div>
                    <div class="detail-text">{{ selectedLexiconIdiom.story || selectedLexiconIdiom.background || '暂无故事' }}</div>
                  </div>
                  <div class="detail-block">
                    <div class="detail-label">记忆桩提示</div>
                    <ul class="memory-list">
                      <li v-for="tip in memoryAid(selectedLexiconIdiom)" :key="tip">{{ tip }}</li>
                    </ul>
                  </div>
                  <div class="detail-block">
                    <div class="detail-label">使用示例</div>
                    <ul class="usage-examples-list" v-if="getUsageExamples(selectedLexiconIdiom).length">
                      <li v-for="(example, idx) in getUsageExamples(selectedLexiconIdiom)" :key="idx">
                        {{ example }}
                      </li>
                    </ul>
...
import {
  ...
  getIdiomsByTag,
  searchIdiom
} from '../../api/attentionIdiomAdvanced.js'

const router = useRouter()
const sidebarOpen = ref(false)
const promptInput = ref('人物成长、逆袭和坚韧')
const domainInput = ref('学习 / 演讲 / 写作')
const tagsInput = ref('励志,成长,处世')
const idiomCount = ref('8')
const difficulty = ref('进阶')
const memorizeSeconds = ref('90')

const isLoadingIdioms = ref(false)
...
const idiomKey = (idiom) => idiom.id || idiom.idiom || Math.random().toString(36).slice(2)
...
const normalizeIdioms = (list = []) =>
  list.map((item, idx) => ({
    id: item.id || idx + 1,
    idiom: item.idiom || item.word || `成语${idx + 1}`,
    pinyin: item.pinyin || '',
    literalMeaning: item.literalMeaning || item.literal_meaning || '',
    meaning: item.meaning || item.explanation || '',
    moral: item.moral || item.motto || '',
    originDynasty: item.originDynasty || item.dynasty || '',
    originSource: item.originSource || item.source || '',
    background: item.background || '',
    story: item.story || '',
    protagonist: item.protagonist || '',
    relatedPersons: item.relatedPersons || '',
    usage: item.usage || item.usageText || '',
    memoryCues: item.memoryCues || item.cues || '',
    tags: item.tags || '',
    videoUrl: item.videoUrl || ''
  }))

const buildQuiz = () => {
  const optionsPool = idioms.value.map((i) => i.idiom)
  quiz.value = idioms.value.map((idiom) => {
    const prompt =
      idiom.moral ||
      idiom.meaning ||
      idiom.background ||
      idiom.story?.slice(0, 40) ||
      '根据提示选择成语'
    const others = optionsPool.filter((opt) => opt !== idiom.idiom)
    const distractors = shuffle(others).slice(0, Math.min(3, others.length))
    const options = shuffle([...distractors, idiom.idiom])
    return {
      idiom,
      prompt,
      options,
...
const saveRecord = async (correct, total, accuracy, duration) => {
  try {
    ...
    await saveIdiomRecord({
      prompt: promptInput.value,
      idiomIds: idioms.value.map((i) => i.id).join(','),
      idiomList: JSON.stringify(idioms.value),
...
const generateAndStart = async () => {
  ...
  try {
    ...
    const payload = {
      ...
      tags,
      count: parseInt(idiomCount.value, 10) || 6,
      difficulty: difficulty.value
    }
    const res = await generateIdiomList(payload)
    const data = res?.data || res || []
    idioms.value = normalizeIdioms(data)
    buildQuiz()
    startMemorize()
    fetchLeaderboard()
  } catch (error) {
    console.warn('生成成语失败：', error?.message || error)
  } finally {
    isLoadingIdioms.value = false
  }
}

const loadLatestIdioms = async () => {
  isLoadingIdioms.value = true
  try {
    const res = await getLatestIdioms(parseInt(idiomCount.value, 10) || 8)
    const data = res?.data || res || []
    idioms.value = normalizeIdioms(data)
    buildQuiz()
    startMemorize()
...
const fetchLeaderboard = async () => {
  try {
    const res = await getIdiomTopRecords(10)
...
const fetchTagsAndIdioms = async () => {
...
const fetchIdiomsBySelectedTag = async () => {
  ...
  lexiconLoading.value = true
  try {
    const res = await getIdiomsByTag(selectedTag.value, 30)
    const data = res?.data || res || []
    tagIdioms.value = normalizeIdioms(data)
    selectedLexiconIdiom.value = tagIdioms.value[0] || null
  } catch (error) {
    console.warn('按标签获取成语失败', error)
    tagIdioms.value = []
    selectedLexiconIdiom.value = null
  } finally {
    lexiconLoading.value = false
...
const selectLexiconIdiom = (idiom) => {
  selectedLexiconIdiom.value = idiom
  lexiconMode.value = 'focus' // 默认进入专注记忆模式
}

const enterFocusMode = (idiom) => {
  selectedLexiconIdiom.value = idiom
  lexiconMode.value = 'focus'
}

const enterVideoMode = (idiom) => {
  if (!idiom.videoUrl) {
    console.warn('该成语没有视频')
    return
  }
  selectedLexiconIdiom.value = idiom
...
const addIdiomToList = (idiom) => {
  const exists = tagIdioms.value.some(item => item.idiom === idiom.idiom)
  if (!exists) {
    tagIdioms.value = [idiom, ...tagIdioms.value]
  }
}

const handleSearch = async () => {
  const idiomText = searchInput.value.trim()
  if (!idiomText) {
    return
  }
  ...
  try {
    const res = await searchIdiom(idiomText)
    const data = res?.data || res
    if (!data) {
      console.warn('检索结果为空')
      return
    ...
    const normalized = normalizeIdioms([data])[0]
    
    // 如果检索到的成语有标签，尝试选中对应的标签并获取该标签下的成语列表
    if (normalized.tags) {
    ...
    // 确保检索到的成语在列表中
    addIdiomToList(normalized)
    
    // 更新选中的成语，默认进入专注记忆模式
    selectedLexiconIdiom.value = normalized
    lexiconMode.value = 'focus'
  } catch (error) {
    console.error('检索成语失败：', error)
...
const getUsageExamples = (idiom) => {
  if (!idiom) return []
  const usageText = idiom.usage || idiom.usageText || ''
  if (!usageText) return []
...
const memoryAid = (idiom) => {
  const tips = []
  if (idiom.memoryCues) tips.push(idiom.memoryCues)
  if (idiom.story) tips.push(`故事画面：${idiom.story.slice(0, 60)}...`)
  if (idiom.moral) tips.push(`意义锚点：${idiom.moral}`)
  if (idiom.literalMeaning) tips.push(`字面拆解：${idiom.literalMeaning}`)
  if (!tips.length) tips.push('尝试谐音、地点桩、人物情绪三步法来记忆。')
...
.idiom-row.mode-focus {
  border-color: rgba(52, 152, 219, 0.6);
  background: rgba(52, 152, 219, 0.1);
}

.idiom-row.mode-video {
...
.idiom-video-btn:hover {
  background: rgba(231, 76, 60, 0.25);
  border-color: rgba(231, 76, 60, 0.6);
  transform: scale(1.1);
}

.idiom-video-btn.active {
  background: rgba(231, 76, 60, 0.3);
