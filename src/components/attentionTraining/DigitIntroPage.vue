<template>
  <div class="digit-intro-container">
    <!-- 背景层，与全局页面保持一致 -->
    ...
    <main class="main-content">
      <header class="page-header">
        <div>
          <div class="badge">数字入门 · 专注力</div>
          <h1 class="page-title">数字入门方格游戏</h1>
          <p class="subtitle">设置一个数字 N，生成 N×N 方格，填入 1 到 N² 的唯一数字。进入后自动计时，点击完成即可结算成绩并写入 TOP10 榜单。</p>
        </div>
        ...
        <div class="game-column">
          <section class="controls">
            ...
            <div class="control-item compact">
              ...
              <div class="next-number">#{{ expectedNumber }}</div>
            </div>
          ...
          <section class="grid-card">
            ...
            <div class="tips">
              <p>规则：点击数字 1 开始，按递增顺序点击到 {{ gridSize * gridSize }}。计时在开始后持续，直到所有数字点击完毕。</p>
              <p class="secondary">提示：每次开始都会打乱数字；刷新不清空已存的排行榜数据。</p>
            </div>
          </section>
        </div>
      </div>
    ...
    <Sidebar
      :is-open="sidebarOpen"
      :active-route="ROUTES.ATTENTION_DIGIT_INTRO"
...
import { getDigitIntroTopRecords, saveDigitIntroRecord } from '../../api/attentionDigitIntro.js'
...
const progress = computed(() => {
  ...
  return ((expectedNumber.value - 1) / totalCells.value) * 100
})
...
const generateGrid = () => {
  ...
  gridNumbers.value = shuffle(numbers)
...
const resetGame = () => {
  ...
  clickedSet.value = new Set()
  generateGrid()
  // 重置后计时归0并停止，等待点击数字1开始计时
}

const handleCellClick = (num) => {
  // 只有在点击数字1时才开始计时
  if (num === 1 && !timer.isRunning && expectedNumber.value === 1) {
    startGame()
  }

  if (num !== expectedNumber.value) {
    return
  }
  ...
  if (expectedNumber.value > totalCells.value) {
    finalizeGame()
  }
}
...
const finalizeGame = async () => {
  ...
  try {
    await saveDigitIntroRecord(payload)
  } catch (error) {
...
const fetchLeaderboard = async () => {
  ...
  try {
    const res = await getDigitIntroTopRecords(gridSize.value, 10)
...
const cellClass = (num) => {
  if (clickedSet.value.has(num)) return 'clicked'
  return ''
}
