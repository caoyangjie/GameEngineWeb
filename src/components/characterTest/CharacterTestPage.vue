    ...
    <main class="test-main-content">
      ...
      <div v-if="currentView === 'list'" class="list-panel">
        ...
        <div class="test-list">
          ...
          <div v-if="testList.length === 0" class="empty-list">
      ...
      <div v-else-if="currentView === 'test'" class="test-panel">
        ...
        <div class="test-header">
          ...
          <div class="test-actions">
            <button class="action-btn toggle-pinyin-btn" @click="togglePinyin">
              {{ settings.showPinyin ? t('characterTest.hidePinyin') : t('characterTest.showPinyin') }}
            </button>
            <button class="action-btn save-btn" @click="saveTest">
              {{ t('characterTest.saveAndBack') }}
            </button>
            <button class="action-btn reset-btn" @click="resetTest">
          ...
          <div class="characters-grid">
            ...
            >
              <div class="character-wrapper">
                <div class="character-text" @click="toggleCharacterStatus(char)">
                  {{ char.character }}
                </div>
                ...
                <div class="character-status">
                  <span 
                    ...
                    @click.stop="toggleCharacterStatus(char)"
                  >✓</span>
                  <span 
...
import { getTestCharacters, saveTestRecord, getAllTestRecords, deleteTestRecord } from '../../api/characterTest.js'
...
const startTest = async () => {
  try {
    ...
    if (response.code === 200 && response.data) {
      ...
      alert(response.msg || t('characterTest.loadFailed'))
    }
  } catch (error) {
    console.error('加载测试字符失败:', error)
    alert(error.message || t('characterTest.loadFailed'))
  }
}
...
const saveTest = async () => {
  if (!settings.studentName.trim()) {
    alert(t('characterTest.pleaseEnterStudentName'))
    return
  }
  ...
  try {
    ...
    if (response.code === 200) {
      alert(t('characterTest.saveSuccess'))
...
const deleteTest = async (id) => {
  if (confirm(t('characterTest.confirmDelete'))) {
    try {
...
const resetTest = () => {
  if (confirm(t('characterTest.confirmReset'))) {
    testCharacters.value = []
    currentView.value = 'list'
    currentTestId.value = null
  }
