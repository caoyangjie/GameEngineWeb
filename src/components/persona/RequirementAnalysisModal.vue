<template>
  <Transition name="modal-fade">
    <div v-if="modelValue" class="modal-overlay" @click="handleOverlayClick">
      <div 
        class="modal-content requirement-analysis-modal" 
        :class="{ 'is-maximized': isMaximized }"
        @click.stop
      >
        <!-- 头部 -->
        <div class="modal-header">
          <h3 class="modal-title">{{ t('persona.requirementAnalysis.title') }}</h3>
          <div class="header-actions">
            <button 
              class="header-btn" 
              @click="toggleMaximize"
              :title="isMaximized ? t('persona.requirementAnalysis.restore') : t('persona.requirementAnalysis.maximize')"
            >
            ...
            <button 
              ...
              @click="handleClose"
              :title="t('persona.requirementAnalysis.close')"
            >
          ...
          <div class="editor-container">
            ...
            <div class="editor-panel">
              <div class="panel-header">
                <span class="panel-title">{{ t('persona.requirementAnalysis.editor') }}</span>
              </div>
              <div class="panel-content">
                <textarea
                  ...
                  :placeholder="t('persona.requirementAnalysis.editorPlaceholder')"
                ></textarea>
              </div>
            </div>
            ...
            <div class="preview-panel">
              <div class="panel-header">
                <span class="panel-title">{{ t('persona.requirementAnalysis.preview') }}</span>
              </div>
              <div 
        ...
        <div class="modal-footer">
          <button class="btn-secondary" @click="handleClose">
            {{ t('persona.requirementAnalysis.cancel') }}
          </button>
          <button class="btn-primary" @click="handleSave">
            {{ t('persona.requirementAnalysis.save') }}
          </button>
        </div>
      </div>
    </div>
...
renderer.code = function(code, language, escaped) {
  ...
  if (language === 'plantuml' || language === 'puml') {
    ...
    try {
      ...
      const errorMsg = t('persona.requirementAnalysis.diagramLoadError')
        return `<div class="diagram-container plantuml-container" data-type="plantuml" data-id="${id}" data-code="${encodeURIComponent(rawCode)}" data-url="${plantumlUrl}"><div class="plantuml-loading">${t('persona.requirementAnalysis.loading') || '加载中...'}</div></div>`
    } catch (e) {
      console.error('PlantUML encoding error:', e)
        return `<div class="diagram-container plantuml-container" data-type="plantuml" data-id="${id}"><div class="diagram-error">${t('persona.requirementAnalysis.diagramLoadError')}: ${e.message || '编码失败'}</div></div>`
    }
  }
...
const renderedMarkdown = computed(() => {
  if (!markdownContent.value) {
    return '<p class="empty-preview">' + t('persona.requirementAnalysis.emptyPreview') + '</p>'
  }
  try {
    return marked.parse(markdownContent.value)
  } catch (error) {
    console.error('Markdown parsing error:', error)
    return '<p class="error-preview">' + t('persona.requirementAnalysis.renderError') + '</p>'
  }
})
    ...
    try {
      ...
      if (!code) {
        console.warn('Mermaid 代码为空')
              container.innerHTML = `<div class="diagram-error">${t('persona.requirementAnalysis.mermaidError')}: 代码为空</div>`
      ...
      try {
        ...
        container.innerHTML = `<div class="diagram-error">${t('persona.requirementAnalysis.mermaidError')}: ${error.message || 'Unknown error'}</div>`
        container.dataset.rendered = 'error'
      }
    } catch (error) {
  ...
  for (const container of plantumlContainers) {
    ...
    try {
      ...
      if (url) {
        ...
        img.onerror = () => {
          container.innerHTML = `<div class="diagram-error">${t('persona.requirementAnalysis.diagramLoadError')}</div>`
          container.dataset.rendered = 'error'
      ...
      container.innerHTML = `<div class="diagram-error">${t('persona.requirementAnalysis.diagramLoadError')}: ${error.message || 'Unknown error'}</div>`
      container.dataset.rendered = 'error'
    }
  }
  ...
  for (const container of mindmapContainers) {
    ...
    if (svg && code) {
      try {
        ...
        container.innerHTML = `<div class="diagram-error">${t('persona.requirementAnalysis.mindmapError') || '思维导图渲染失败'}: ${error.message || 'Unknown error'}</div>`
        container.dataset.rendered = 'error'
      }
    } else {
      console.warn('Markmap 容器或代码缺失，svg:', !!svg, 'code:', !!code)
      if (!code) {
        container.innerHTML = `<div class="diagram-error">${t('persona.requirementAnalysis.mindmapError') || '思维导图渲染失败'}: 代码为空</div>`
        container.dataset.rendered = 'error'
      }
    }
  }
...
watch(() => props.initialContent, (newVal) => {
  // 只在弹窗打开时同步内容，避免关闭时覆盖用户编辑的内容
  if (props.modelValue && newVal !== undefined && newVal !== null) {
...
.requirement-analysis-modal.is-maximized {
  width: 100%;
  height: 100vh;
  max-width: 100%;
  max-height: 100vh;
